...
> **Important:** `/bootstrap` performs file content reads (`cat`) and creates configuration files (`Write`/`Edit`) that are blocked by the thin-orchestrator hooks (`read-route.sh`, `edit-route.sh`, `bash-route.sh`) once active.
>
> **If thin-orchestrator hooks are already active**, temporarily disable them before running this command:
>
> ```bash
> mv .claude/settings.json .claude/settings.json.bak
> # then run claude "/bootstrap" in a fresh session
> mv .claude/settings.json.bak .claude/settings.json
> ```
>
> `/bootstrap` is intended for initial template setup, before hooks are wired in.
