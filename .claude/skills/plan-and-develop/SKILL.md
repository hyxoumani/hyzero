...
Dispatch `analyst` to explore the affected code (trace imports, read function bodies, find tests, understand data flow).

Brief: write findings to `docs/plans/{feature-name}/research.md`:
...
Dispatch `analyst` (analyst can write to `docs/plans/` per its contract). Brief: based on the Phase 1 research, write `docs/plans/{feature-name}/plan.md`:
...
Dispatch a second `analyst` (fresh context — no Phase 2 author bias) to review the plan for gaps, missing edge cases, overengineering, or assumptions that don't hold against the codebase.

Iterate internally on REJECT (same pattern as `/verify`): re-dispatch analyst with the issues to revise `plan.md`, then re-dispatch reviewer. Loop until APPROVE. The user does not see iteration.
