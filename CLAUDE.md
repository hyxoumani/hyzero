...
`hyzero` is a Rust chess engine attempting to emulate MuZero. It uses bitboard representation and magic bitboards for sliding piece move generation, with an async TCP server for networked play. Includes MCTS tree search, self-play infrastructure, inference batching, and a replay buffer. Python neural network layer is partially implemented (Task 24 done, Tasks 25-26 pending).
...
- `config.py` — DEFAULT_CONFIG with hidden_channels=64, num_res_blocks=4, action space 4096
- `models/common.py` — `ResidualBlock` (Conv→BN→ReLU→Conv→BN + skip → ReLU)
- `models/representation.py` — `RepresentationNetwork` (h): [B,19,8,8] → [B,64,8,8]
- `models/dynamics.py` — `DynamicsNetwork` (g): hidden + action_planes → (next_hidden, reward)
- `models/prediction.py` — `PredictionNetwork` (f): hidden → (policy_logits [B,4096], value [B,1])
